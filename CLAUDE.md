...
- **8 master faders profondeur-aware** (section `[master_faders]`) : F1 KR Avant / F2 KR Arrière / F3 Minibeam / F4 Lyre / F5 PAR Ambiance / F6 Vitesse / F7 Laser / F8 Master. Chaque fader liste `id,canal|...`. **Pas de bindings `faderN_midi`** : SweetLight mappe nativement les 8 faders de l'APC40 mkII aux 8 master faders dans l'ordre. Le script **supprime** toute clé `faderN_midi` (les forcer à `canal N, CC7` cassait 7 faders sur 8 — tous les backups fonctionnels avaient 0 binding). `fade_time = 0` est forcé pour un focus réactif (effet de bord assumé : les fondus deviennent des coupures sèches).
