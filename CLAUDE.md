...
- **`build_page_block`** : `title` affiché caché quand il y a une image, SAUF pages `FX`/`MANUEL`/`STROBE` (icône pas assez parlante) et `FORCE_TITLE` (texte court imposé). `fader = yes` si titre dans `FADER_BUTTONS` ; sinon `masterspeedfader = 1` si le fichier est un `.gpj` **ou** si le titre est dans `SPEED_TITLES` (scènes `.scex` animées dont la vitesse doit suivre le fader Vitesse : effets DJ LIVE L3 sauf BUILD, chases/paires FX).
- **Réécriture de `live.ini`** : remplace tout de `[page1]` à `[board]` par nos blocs ; **`[page] number` = NOMBRE de pages** (⚠️ pas l'onglet actif — mettre moins que le nombre réel de blocs `[pageN]` fait **supprimer** les pages en trop par SweetLight au chargement suivant). Puis :
  - **`[master_faders]`** entièrement réécrit : F0 **Vitesse** (`type_fader0 = 1` = *speed*, liste vide → scale la vitesse de **tout ce qui est animé** : `.gpj` de mouvement + effets `.scex` marqués `SPEED_TITLES`, tous en `masterspeedfader = 1`), F1 Puissance faisceau (BSW+PAR dimmer), F2 Hazer Fog, F3 Hazer Fan.
...
- **Vitesse (mouvements ET effets)** : plus de boutons Lent/Rapide — chaque `.gpj` de mouvement + chaque effet animé de `SPEED_TITLES` a `masterspeedfader = 1` et suit le **même** master fader **Vitesse** (type *speed*). La vitesse propre de la scène = le « 100 % ». SweetLight n'a qu'un seul fader *speed* actif (`masterspeedfader = 1` = booléen « suit la vitesse », pas un index) → un seul fader physique pilote toute l'animation.
